package com.johnson.arcface2camerax.nativeface.faceserver;
...
import com.johnson.arcface2camerax.nativeface.model.FaceRegisterInfo;
...
public class FaceServer {
     ...
     * 对比两个特征码的相似度
     */
    public float comparisonCode(byte[] firstCode, byte[] secondCode) {
        if (firstCode == null || secondCode == null) return 0;
        FaceSimilar faceSimilar = new FaceSimilar();
        FaceFeature firstFaceFeature = new FaceFeature(firstCode);
        FaceFeature secondFaceFeature = new FaceFeature(secondCode);
        FaceSimilar faceSimilars = new FaceSimilar();
        float maxSimilar = 0;
        synchronized (faceEngine.getClass()) {
            faceEngine.compareFaceFeature(firstFaceFeature, secondFaceFeature, faceSimilars);
